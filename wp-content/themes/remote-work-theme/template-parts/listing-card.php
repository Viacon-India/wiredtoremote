...
<div class="blog-card">
    <a href="<?php echo get_the_permalink($post_ID); ?>">
        <figure class="blog-card-figure">
            <?php if (has_post_thumbnail()) : ?>
                <?php echo get_the_post_thumbnail($post_ID, 'listing-thumbnail', array('class' => 'img-responsive')); ?>
            <?php else : ?>
    ...
    <div class="blog-card-content">
        <?php if(!empty($cat)) { 
            echo '<a class="blog-card-cat" href="'.esc_url(get_category_link($cat[0]->term_id)).'" title="'.$cat[0]->cat_name.'">'.$cat[0]->cat_name.'</a>';
        } ?>
